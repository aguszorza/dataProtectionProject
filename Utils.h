...
public:
    ...
    static std::vector< std::vector<Mpz> > addTattoo(const std::vector< std::vector<Mpz> >& matrix,
            const std::vector<Difference>& differences, const DataHider& dataHider, Generator& generator_r2,
            std::string w, const int& column_1, const int& column_2, const int& EP);
};
